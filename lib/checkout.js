...
import { iranStateCode } from './iran-states.js';
...
export function validAddress(value) {
  ...
  const state = iranStateCode(address.state);
  if (!address.first_name || !address.last_name || !address.address_1 || !address.city || !state || !address.phone || !/^\d{10}$/.test(address.postcode) || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(address.email)) return null;
  if (fields.some(key => address[key].length > 200)) return null;
  return { ...address, state, country: 'IR' };
}

export const ZIBAL_METHOD = 'WC_Gateway_Zibal';

export function availableMethods(cart, sandbox = false) {
  return (cart?.payment_methods || []).filter(method => sandbox ? method === ZIBAL_METHOD : ['cod', ZIBAL_METHOD].includes(method));
}
