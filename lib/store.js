...
export async function getProducts() {
  ...
  if (!base) return { products: demoProducts, mode: 'demo', error: '' };
  try {
    const url = new URL(base);
    if (url.protocol !== 'https:' && !(url.protocol === 'http:' && url.hostname === 'localhost')) throw new Error('نشانی فروشگاه باید HTTPS باشد.');
    const response = await fetch(`${url.origin}/wp-json/wc/store/v1/products?per_page=24`, { cache: 'no-store', signal: AbortSignal.timeout(8000) });
