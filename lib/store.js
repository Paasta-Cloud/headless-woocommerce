...
export async function getProducts() {
  try {
    ...
    const response = await fetch(`${origin}/wp-json/wc/store/v1/products?per_page=24`, { cache: 'no-store', signal: AbortSignal.timeout(8000) });
...
export async function getProduct(id) {
  ...
  const response = await fetch(`${origin}/wp-json/wc/store/v1/products/${id}`, { cache: 'no-store', signal: AbortSignal.timeout(8000) });
