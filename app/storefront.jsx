...
export default function Storefront({ products, mode, error }) {
  ...
  const { cart, entries, subtotal, unit: cartUnit, busy, error: cartError, changeQuantity } = useCart(mode);
  ...
  const total = mode === 'live' && subtotal !== null ? subtotal : products.reduce((sum, product) => sum + product.price * (cart[product.id] || 0), 0);
  const unit = mode === 'live' && cartUnit ? cartUnit : products[0]?.unit || 'تومان';
  const price = value => new Intl.NumberFormat('fa-IR').format(value) + ' ' + unit;
  const cartLines = mode === 'live' ? entries : products.filter(product => cart[product.id]);

  return <>
    <div className="announcement">نمونهٔ متن‌باز فروشگاه فارسی <span>·</span> {mode === 'demo' ? 'قیمت‌ها و کالاهای حالت نمایشی واقعی نیستند' : 'قیمت و موجودی نهایی در سبد ووکامرس تأیید می‌شوند'}</div>
    <header className="site-header"><div className="header-inner">
      ...
      <a className="header-link" href="/account">حساب کاربری</a><a className="cart-trigger" href="/cart" aria-label={`سبد خرید، ${count} کالا`}><Icon name="bag"/><span>سبد خرید</span><b>{count.toLocaleString('fa-IR')}</b></a>
    </div></header>
    <main>
      ...
      {error ? <div className="notice" role="alert">{error}</div> : visible.length ? <div className="product-grid">{visible.map(product => <article className="product" key={product.id}><a href={`/product/${product.id}`} className={`product-image ${product.tone}`}>{product.image ? <img src={product.image} alt={product.name} loading="lazy"/> : <span>{product.label}</span>}</a><div className="product-info"><span>{product.category}</span><h3><a href={`/product/${product.id}`}>{product.name}</a></h3><p>{product.description.replace(/<[^>]*>/g, '')}</p><div className="product-bottom"><strong>{price(product.price)}</strong><button type="button" disabled={busy || product.purchasable === false} onClick={async () => { await changeQuantity(product.id, 1); setCartOpen(true); }} aria-label={product.purchasable === false ? `${product.name} فعلاً قابل افزودن نیست` : `افزودن ${product.name} به سبد`} title={product.purchasable === false ? 'این کالا ناموجود است یا انتخاب گزینه می‌خواهد' : undefined}><Icon name="plus"/></button></div></div></article>)}</div> : <div className="empty">کالایی با این جست‌وجو پیدا نشد. عبارت دیگری امتحان کنید.</div>}</section>
    ...
    {cartOpen && <div className="drawer-layer"><button className="drawer-backdrop" type="button" aria-label="بستن سبد خرید" onClick={() => setCartOpen(false)}></button><aside className="drawer" role="dialog" aria-modal="true" aria-label="سبد خرید"><div className="drawer-head"><h2>سبد خرید <span>({count.toLocaleString('fa-IR')})</span></h2><button type="button" onClick={() => setCartOpen(false)} aria-label="بستن"><Icon name="close"/></button></div><div className="drawer-items">{cartError && <p className="cart-error" role="alert">{cartError}</p>}{count ? cartLines.map(product => <div className="cart-item" key={product.id}><div className={`cart-thumb ${product.tone || 'stone'}`}>{product.image ? <img src={product.image} alt=""/> : product.label || product.name?.slice(0, 2)}</div><div><strong>{product.name}</strong><span>{price(product.price)}</span><div className="quantity"><button type="button" disabled={busy} aria-label={`کم کردن ${product.name}`} onClick={() => changeQuantity(product.id, -1)}><Icon name="minus"/></button><b>{cart[product.id].toLocaleString('fa-IR')}</b><button type="button" disabled={busy} aria-label={`افزودن ${product.name}`} onClick={() => changeQuantity(product.id, 1)}><Icon name="plus"/></button></div></div></div>) : <p className="cart-empty">سبد شما خالی است. از میان کالاها چیزی انتخاب کنید.</p>}</div><div className="drawer-foot"><div><span>جمع کالاها</span><strong>{price(total)}</strong></div><p>{mode === 'demo' ? 'این سبد فقط در همین صفحه نگهداری می‌شود.' : 'سبد با ووکامرس هماهنگ است.'} پرداخت هنوز فعال نیست و سفارشی ثبت نمی‌شود.</p><button type="button" onClick={() => setCartOpen(false)}>ادامهٔ دیدن کالاها</button></div></aside></div>}
  </>;
}
