...
export default function Storefront({ products, mode, error }) {
  ...
  return <>
    ...
    <main>
      ...
      {error ? <div className="notice" role="alert">{error}</div> : visible.length ? <div className="product-grid">{visible.map(product => <article className="product" key={product.id}><div className={`product-image ${product.tone}`}>{product.image ? <img src={product.image} alt={product.name} loading="lazy"/> : <span>{product.label}</span>}</div><div className="product-info"><span>{product.category}</span><h3>{product.name}</h3><p>{product.description.replace(/<[^>]*>/g, '')}</p><div className="product-bottom"><strong>{price(product.price)}</strong><button type="button" disabled={busy || product.purchasable === false} onClick={async () => { await changeQuantity(product.id, 1); setCartOpen(true); }} aria-label={product.purchasable === false ? `${product.name} فعلاً قابل افزودن نیست` : `افزودن ${product.name} به سبد`} title={product.purchasable === false ? 'این کالا ناموجود است یا انتخاب گزینه می‌خواهد' : undefined}><Icon name="plus"/></button></div></div></article>)}</div> : <div className="empty">کالایی با این جست‌وجو پیدا نشد. عبارت دیگری امتحان کنید.</div>}</section>
